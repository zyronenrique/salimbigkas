    ...
    <div className="relative bg-[#F8F8F8] h-screen">
      <div className="relative flex flex-col items-center justify-center w-full h-full">
        ...
        <div className="relative w-full max-w-4xl mx-auto">
          <img
            loading="lazy"
            src={imageSrc.seatwork}
            alt="Seatwork"
            className="hidden md:block w-full h-auto object-contain"
          />
          <motion.button
            disabled={state.isLoading}
            title="Start Seatwork"
            type="button"
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate(`/${role?.toLowerCase()}/${formattedGradeLevel}/seatworks/select-seatwork`)}
          >
            {state.isLoading ? (
              <SpinLoading size={20} />
            ) : (
...
SeatworkHome.displayName = 'SeatworkHome';

export default SeatworkHome;